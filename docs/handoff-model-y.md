...
- **A — `st-5em` DONE.** stelis `model-y`: `site` out of the graph (committed,
  pushed). beeatlas `model-y` (uncommitted): `npm run fetch-data` = the Stelis
...
- **B — `st-mfb` DONE** (beeatlas `model-y`, uncommitted). The runtime set is
...
- **C — `st-jeu` code DONE** (beeatlas `model-y`, uncommitted; **not yet run
  on maderas**). `nightly.sh` rewritten: publish flock (`var/publish.lock`,
...
- **D — `st-pry` code DONE** (beeatlas `model-y`, uncommitted; **not
  deployed**). `PipelineBackupBucket` added to the stack mirroring the notes
...
- **E — `st-nee`** (beeatlas API, PRODUCTION): the write path — commit → the
  SAME `var/publish.lock` flock nightly now takes → fetch-data notes-only
  (`STELIS_REBUILD_KEYS`) → build → merge-swap → live/pending. Depends on C.
