...
**D2. Leafness is READ from provenance, not declared beside it.** An artifact is
expected to be a leaf by KIND (`'code`, `'external` — producerless by definition)
or by ORIGIN (`'authoritative`, `'upstream`). Rejected: a `#:leaf?` flag. It would
...
- **The check is invoked by convention** — from `beeatlas.rkt` at module load,
  which is why it reaches `--why` and `--explain` and not only `'build`. Nothing
  makes a *second* graph call it, and the failure mode there is silence. Tracked
  as **st-0kf**; the site st-zb9 originally proposed (main.rkt's validation block)
  has the same weakness and a narrower reach, so this is an open edge rather than
  a settled placement.
